...
bool UAdvKitBuildModule_Wall::CreateTransitionBetween_Implementation(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* SourceZone, AAdvKitZone* TargetZone)
{
	...
	//Create transition
	auto Transition = CreateTransitionLine(this, "Corner", SourceZone, TransitionSourceBottom, TransitionTargetBottom, TransitionSourceTop, TransitionTargetTop, ForCharacterClass, TargetZone->GetPhysics(), TargetZone);
	if (!Transition)
	{
		return false;
	}
...
void UAdvKitBuildModule_Wall::CreateExitTransitions(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* ForZone)
{
	...
	if (bCreateTransitionToFloor)
	{
		...
		auto Transition = CreateTransitionLine(this, "ExitFloor", ForRectZone, TransitionStart1, TransitionEnd1, TransitionStart2, TransitionEnd2, ForCharacterClass, EAdvKitMovementMode::Walking);
		if (Transition)
		{
	...
	if (bCreateTransitionToPlatform)
	{
		...
		auto Transition = CreateTransitionLine(this, "ExitPlatform", ForRectZone, TransitionStart1, TransitionEnd1, TransitionStart2, TransitionEnd2, ForCharacterClass, EAdvKitMovementMode::Walking);
		if (Transition)
		{
			Transition->TransitionDirection = FVector::UpVector;
		}
