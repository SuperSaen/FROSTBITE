...
void AAdvKitInventoryItem::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	auto* Character = Cast<AAdvKitCharacter>(GetInstigator());
...
void AAdvKitInventoryItem::OnEquip_Implementation(bool bInstant)
{
	...
	AAdvKitCharacter* AdvOwner = Cast<AAdvKitCharacter>(GetInstigator());
	if (AdvOwner)
	{
...
{
	AAdvKitCharacter* Character = Cast<AAdvKitCharacter>(GetInstigator());
	if (Character)
	{
...
void AAdvKitInventoryItem::OwnerDied_Implementation()
{
	...
	if (bDropOnDeath)
	{
		Drop(GetInstigator()->GetVelocity());
	}
...
void AAdvKitInventoryItem::Drop_Implementation(FVector Velocity)
{
	...
	FVector SpawnLocation = GetInstigator() ? GetInstigator()->GetActorLocation() : GetActorLocation();
	FRotator SpawnRotation = GetInstigator() ? GetInstigator()->GetActorRotation() : GetActorRotation();
