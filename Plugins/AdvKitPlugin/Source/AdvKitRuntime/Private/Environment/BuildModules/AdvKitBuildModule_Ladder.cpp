...
bool UAdvKitBuildModule_Ladder::GatherPotentialTargetZones_Implementation(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* ForZone, TArray<AAdvKitZone*>& OutZones)
{
	...
	float MaxJumpDistance = FMath::Max<float>(MovementComp->LadderJumpDistanceHorizontal, MovementComp->LadderJumpDistanceBack);
...
bool UAdvKitBuildModule_Ladder::CreateTransitionBetween_Implementation(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* SourceZone, AAdvKitZone* TargetZone)
{
	...
	FString TransitionId("SideJump");
	bool bNeedsJump = bSidewaysTransitionRequiresJump;
	...
	else if (ForwardDot <= (-1.0f + ErrorMarginDot))
	{
		//Ladder is on the same Y plane
		if (FMath::Abs<float>(LocalLocationOther.Y) > ErrorMarginPosition)
		{
			return false;
		}

		//Ladder is out of reach
		if (FMath::Abs<float>(LocalLocationOther.X) > MovementComp->LadderJumpDistanceBack + ErrorMarginPosition)
		{
			return false;
		}

		TransitionId = FString("BackJump");
		bNeedsJump = true;
	}
	else
	{
	...
	{
		auto Transition = CreateTransitionPoint(this, TransitionId,SourceZoneLine,StartTop, EndTop, ForCharacterClass, TargetZoneLine->GetPhysics(), TargetZoneLine);
		if (Transition)
		{
			Transition->bNeedsJump = bNeedsJump;
			return true;
		}
	}
	...
	else if (MaxZOverlap > MinZOverlap)
	{
		...
		auto Transition = CreateTransitionLine(this, TransitionId,SourceZoneLine, StartTop, EndTop, StartBottom, EndBottom, ForCharacterClass, TargetZoneLine->GetPhysics(), TargetZoneLine);
		if (Transition)
		{
			Transition->bNeedsJump = bNeedsJump;
			return true;
...
void UAdvKitBuildModule_Ladder::CreateExitTransitions(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* ForZone)
{
	...
	if (bCreateTransitionToFloor)
	{
		...
		auto Transition = CreateTransitionPoint(this, "ExitFloor", ForLineZone, TransitionStart, TransitionEnd, ForCharacterClass, EAdvKitMovementMode::Walking);
		if (Transition)
		{
	...
	if (bCreateTransitionToPlatform)
	{
		...
		auto Transition = CreateTransitionPoint(this, "ExitTop",ForLineZone,TransitionStart, TransitionEnd, ForCharacterClass, EAdvKitMovementMode::Walking);
		if (Transition)
		{
			Transition->TransitionDirection = FVector::UpVector;
		}
