...
bool UAdvKitBuildModule_Ledge_Tightspace::CreateTransitionLedgeToTightSpace(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* Ledge, AAdvKitZone* TightSpace)
{
	...
	if (SelfMin != SelfMax)
	{
		Transition = CreateTransitionLine(this, "ToTightSpace", LedgeLine, SelfMin, OtherMin, SelfMax, OtherMax, ForCharacterClass, TightSpaceLine->GetPhysics(), TightSpaceLine);
	}
	//Only a single point overlapping
	else if (SelfMin == SelfMax)
	{
		Transition = CreateTransitionPoint(this, "ToTightSpace", LedgeLine, SelfMin, OtherMin, ForCharacterClass, TightSpaceLine->GetPhysics(), TightSpaceLine);
	}
...
bool UAdvKitBuildModule_Ledge_Tightspace::CreateTransitionTightSpaceToLedge(TSubclassOf<AAdvKitCharacter> ForCharacterClass, AAdvKitZone* TightSpace, AAdvKitZone* Ledge)
{
	...
	if (SelfMin != SelfMax)
	{
		Transition = CreateTransitionLine(this, "ToLedge",TightSpaceLine, SelfMin, OtherMin, SelfMax, OtherMax, ForCharacterClass, LedgeLine->GetPhysics(), LedgeLine);
	}
	//Only a single point overlapping
	else if (SelfMin == SelfMax)
	{
		Transition = CreateTransitionPoint(this, "ToLedge", TightSpaceLine, SelfMin, OtherMin, ForCharacterClass, LedgeLine->GetPhysics(), LedgeLine);
	}
